...
export const orderMeal = async (token, mealId, memberId) => {
  return await API.post(`${MEMBER_URL}/order/${mealId}/${memberId}`, null, {
...
export const orderOnProgressMember = async (token, memberId) => {
  return await API.get(`${MEMBER_URL}/order/${memberId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
...
export const orderCompleteMember = async (token, memberId) => {
  return await API.get(`${MEMBER_URL}/order-complete/${memberId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
